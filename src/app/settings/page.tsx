...
export default function SettingsPage() {
  ...
  const [roleFilter, setRoleFilter] = useState<
    'all' | 'SUPERADMIN' | 'ADMIN' | 'MANAGER' | 'AUDITOR' | 'EMPLOYEE' | 'VIEWER'
  >('all')
  ...
  const buildLoginAuditUrl = useCallback(
    (cursor?: string | null) => {
    ...
    async (mode: 'replace' | 'more' = 'replace') => {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json()
          setLoginAudits((prev) =>
            mode === 'more' ? [...prev, ...(data.events || [])] : data.events || []
          )
    ...
    async (mode: 'auto' | 'manual' = 'manual') => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setUsers((prev) =>
            prev.map((user) => (user.id === editingId ? { ...user, ...updated } : user))
          )
          const nextSnapshot = {
          ...
          if (data.requiresApproval) {
            ...
            toastRef.current.success(mode === 'auto' ? 'Автосохранено' : 'Изменения сохранены', {
              id: toastId,
            })
          }
        } else {
  ...
  const deleteUser = async (userId: string) => {
    if (
      !confirm(
        '\У\д\а\л\и\т\ь \п\о\л\ь\з\о\в\а\т\е\л\я? \Э\т\о \д\е\й\с\т\в\и\е \н\е\л\ь\з\я \о\т\м\е\н\и\т\ь.'
      )
    ) {
      return
    }
  ...
  const toggleUserAccess = async (user: User) => {
    ...
    try {
      ...
      toast.success(nextAccess ? '\Д\о\с\т\у\п \о\т\к\р\ы\т' : '\Д\о\с\т\у\п \з\а\к\р\ы\т')
    } catch (error) {
  ...
  const formatBooleanLabel = (value: string | null) => {
    ...
    return value === 'true' ? '\В\к\л\ю\ч\е\н\о' : '\В\ы\к\л\ю\ч\е\н\о'
  }
  ...
  const formatAuditValue = (field: string | null, value: string | null) => {
    ...
    if (field === 'canLogin') {
      return value === 'true' ? '\О\т\к\р\ы\т' : '\З\а\к\р\ы\т'
    }
  ...
  const buildAuditUrl = useCallback(
  ...
  const loadAudit = useCallback(
    async (userId: string, mode: 'replace' | 'more' = 'replace', cursor?: string | null) => {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    const matchesEmail =
      emailFilter === 'all' || (emailFilter === 'has' ? !!user.email : !user.email)

    return matchesSearch && matchesRole && matchesStatus && matchesTelegram && matchesEmail
  })
  ...
  const applyBulkAction = useCallback(async () => {
    ...
    try {
      ...
      if (data.requiresApproval) {
        toast.message(`\Н\у\ж\н\о \п\о\д\т\в\е\р\д\и\т\ь: ${data.pendingApprovals ?? 0}`)
        await loadApprovals()
      }
      await loadUsers()
      clearSelection()
            ...
            {isSuperAdmin && (
              <div className="panel-soft panel-glass mb-6 rounded-2xl p-4">
                ...
                {approvalsLoading ? (
                            ...
                            <div className="space-y-1">
                              ...
                              {approval.action === 'DEMOTE_ADMIN' && approval.payload?.newRole && (
                                <div className="text-xs text-emerald-400">
                                  {'\Н\о\в\а\я \р\о\л\ь:'}{' '}
                                  {formatRoleLabel(approval.payload.newRole)}
                                </div>
                              )}
                  ...
                  <div className="text-xs text-gray-500">
                    {'\Н\е\т \з\а\п\р\о\с\о\в \н\а \п\о\д\т\в\е\р\ж\д\е\н\и\е'}
                  </div>
                )}
              </div>
            )}
            ...
            <div className="panel-soft panel-glass mb-6 rounded-2xl p-4">
              ...
              <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div className="space-y-1 text-xs text-gray-500">
                  <p>{'\Д\л\я \в\х\о\д\а \ч\е\р\е\з Google \т\р\е\б\у\е\т\с\я email.'}</p>
                  {!isSuperAdmin && (
            ...
            {selectedIds.size > 0 && (
              <div className="panel-soft panel-glass mb-6 flex flex-wrap items-center gap-3 rounded-2xl p-4">
                ...
                >
                  ...
                  {isSuperAdmin && <option value="role">{'\С\м\е\н\и\т\ь \р\о\л\ь'}</option>}
                ...
                <button
                  ...
                  disabled={
                    !bulkAction || bulkLoading || selectedIds.size === 0 || bulkDemoteBlocked
                  }
                  className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-white transition hover:bg-emerald-500 disabled:opacity-50"
                >
                  {bulkLoading ? (
                  ...
                  >
                    ...
                    {!isEditing ? (
                      ...
                      <div className="mt-4 grid gap-3 text-sm">
                        ...
                        <div className="grid gap-3 border-t border-white/10 pt-3 text-xs">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <div className="grid gap-1">
                              <span className="text-gray-400">{'\Т\и\х\и\е \ч\а\с\ы (\с)'}</span>
                              <input
                                type="time"
                            ...
                            <div className="grid gap-1">
                              <span className="text-gray-400">{'\Т\и\х\и\е \ч\а\с\ы (\д\о)'}</span>
                              <input
                                type="time"
                        ...
                        {isEditing ? (
                          <>
                            ...
                            <button
                              ...
                              className="p-2 text-emerald-400 transition hover:text-emerald-300"
                            >
                              {isSaving ? (
                                <Loader2 className="h-4 w-4 animate-spin" />
                              ) : (
                          ...
                          <>
                            ...
                            {user.id !== session.user.id && (
                              <button
                                ...
                                className="p-2 text-gray-400 transition hover:text-red-400 disabled:opacity-60"
                              >
                          ...
                          {auditLoading[user.id] && !hasAuditEntries ? (
                            ...
                            <div className="space-y-4">
                              ...
                              {auditCursorByUser[user.id] && (
                                <button
                                  onClick={() =>
                                    loadAudit(user.id, 'more', auditCursorByUser[user.id])
                                  }
                                  className="inline-flex items-center gap-2 text-xs text-emerald-400 transition hover:text-emerald-300"
                                >
          ...
          <div className="panel panel-glass mb-8 rounded-2xl p-6">
            ...
            <div className="mb-4 grid grid-cols-1 gap-3 md:grid-cols-3">
              <select
                ...
                onChange={(e) =>
                  setLoginAuditStatus(e.target.value as 'all' | 'success' | 'failure')
                }
                className={`${fieldBase} w-full px-3 py-2`}
              ...
              <button
                ...
                className="rounded border border-white/10 px-3 py-2 text-gray-300 hover:text-white"
              >
                {'\С\б\р\о\с\и\т\ь'}
              </button>
            </div>
